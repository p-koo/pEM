...
for j = 1:params.numPerturbation
    ...
    diagonals = zeros(length(trackIndex),trackInfo.dimensions);
    correlations = zeros(length(trackIndex),trackInfo.dimensions);
    trackLength = zeros(length(trackIndex),1);
    for i = 1:length(trackIndex)
        bootStrapDeltaX{i} = deltaX{trackIndex(i)};
        diagonals(i,:) = trackInfo.diagonals(trackIndex(i),:);
        correlations(i,:) = trackInfo.correlations(trackIndex(i),:);
        trackLength(i) = trackInfo.trackLength(trackIndex(i));
    end

    % calculate new track properties of resampled data
    bsTrackInfo = trackInfo;
    bsTrackInfo.diagonals = diagonals;
    bsTrackInfo.correlations = correlations;
    bsTrackInfo.trackLength = trackLength;
